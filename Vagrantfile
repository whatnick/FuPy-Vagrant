...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision :shell, :name =>"UDev", :privileged => false, :path => "install_udev_rules.sh"
  config.vm.provision :reload
  config.vm.provision :shell, :name =>"Install Env", :privileged => false, :path => "install_env.sh"
  config.vm.provision :shell, :name =>"Make Firmware", :privileged => false, :path => "make_firmware.sh"
  config.vm.provision :shell, :name =>"Upload Firmware", :privileged => false, :path => "upload_firmware.sh"
end
